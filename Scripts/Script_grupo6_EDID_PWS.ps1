...
$REGION = "us-east-1"
$AMI_ID = "ami-04b4f1a9cf54c11d0" # Ubuntu Server 24.04 en us-east-1 [web:1]
...
$rulesEscaped = $jsonRules.Replace('"', '\"')
...
if ([string]::IsNullOrEmpty($WEB_ACL_ARN)) {
  Write-Host "ERROR: No se ha podido crear el Web ACL."
  exit 1
}

Write-Host "✅ WAF asociado al ALB correctamente"
Write-Host "-----------------------------------------"
